# Copied from: https://github.com/StableCoder/cmake-scripts/blob/main/code-coverage.cmake Modified to output coverage reports.
#
...
# USAGE: To enable any code coverage instrumentation/targets, the single CMake option of `CODE_COVERAGE` needs to be set to 'ON', either by GUI, ccmake, or on
# the command line.
#
# From this point, there are two primary methods for adding instrumentation to targets: 1 - A blanket instrumentation by calling `add_code_coverage()`, where
# all targets in that directory and all subdirectories are automatically instrumented. 2 - Per-target instrumentation by calling
# `target_code_coverage(<TARGET_NAME>)`, where the target is given and thus only that target is instrumented. This applies to both libraries and executables.
#
...
option(LLVM_COV_COMMAND "Which version of llvm-cov to use. (Default is 'llvm-cov')" "llvm-cov")
option(LLVM_PROFDATA_COMMAND "Which version of llvm-profdata to use. (Default is 'llvm-profdata')" "llvm-profdata")

# Programs
find_program(LLVM_COV_PATH ${LLVM_COV_COMMAND})
find_program(LLVM_PROFDATA_PATH ${LLVM_PROFDATA_COMMAND})
...
if (CODE_COVERAGE AND NOT CODE_COVERAGE_ADDED)
    ...
    if (CMAKE_C_COMPILER_ID MATCHES "(Apple)?[Cc]lang" OR CMAKE_CXX_COMPILER_ID MATCHES "(Apple)?[Cc]lang")
        ...
        if (CMAKE_C_COMPILER_ID MATCHES "AppleClang" OR CMAKE_CXX_COMPILER_ID MATCHES "AppleClang")
            # When on macOS and using the Apple-provided toolchain, use the XCode-provided llvm toolchain via `xcrun`
        ...
        if (${CMAKE_VERSION} VERSION_LESS "3.17.0")
            ...
            add_custom_target(
                ccov-clean
                COMMAND ${CMAKE_COMMAND} -E rm -f ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/binaries.list
                COMMAND ${CMAKE_COMMAND} -E rm -f ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/profraw.list
            )
        endif ()
    ...
    if (CODE_COVERAGE)
        ...
        if (target_type STREQUAL "SHARED_LIBRARY" AND target_code_coverage_ALL)
            if (CMAKE_C_COMPILER_ID MATCHES "(Apple)?[Cc]lang" OR CMAKE_CXX_COMPILER_ID MATCHES "(Apple)?[Cc]lang")
                add_custom_target(
                    ...
                    COMMAND ${CMAKE_COMMAND} -E echo "-object=$<TARGET_FILE:${TARGET_NAME}>" >> ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/binaries.list
                    DEPENDS ${TARGET_NAME}
                )
        ...
        if (target_type STREQUAL "EXECUTABLE")
            if (CMAKE_C_COMPILER_ID MATCHES "(Apple)?[Cc]lang" OR CMAKE_CXX_COMPILER_ID MATCHES "(Apple)?[Cc]lang")
                ...
                # Run the executable, generating raw profile data Make the run data available for further processing. Separated to allow Windows to run this
                # target serially.
                add_custom_target(
                    ccov-run-${target_code_coverage_COVERAGE_TARGET_NAME}
                    COMMAND ${CMAKE_COMMAND} -E env ${CMAKE_CROSSCOMPILING_EMULATOR} ${target_code_coverage_PRE_ARGS}
                            LLVM_PROFILE_FILE=${target_code_coverage_COVERAGE_TARGET_NAME}.profraw $<TARGET_FILE:${TARGET_NAME}> ${target_code_coverage_ARGS}
                    COMMAND ${CMAKE_COMMAND} -E echo "-object=$<TARGET_FILE:${TARGET_NAME}>" ${LINKED_OBJECTS} >>
                            ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/binaries.list
                    COMMAND ${CMAKE_COMMAND} -E echo "${CMAKE_CURRENT_BINARY_DIR}/${target_code_coverage_COVERAGE_TARGET_NAME}.profraw" >>
                ...
                add_custom_target(
                    ...
                    COMMAND ${LLVM_COV_PATH} show $<TARGET_FILE:${TARGET_NAME}> -instr-profile=${target_code_coverage_COVERAGE_TARGET_NAME}.profdata
                ...
                add_custom_target(
                    ...
                    COMMAND ${LLVM_COV_PATH} report $<TARGET_FILE:${TARGET_NAME}> -instr-profile=${target_code_coverage_COVERAGE_TARGET_NAME}.profdata
                ...
                add_custom_target(
                    ...
                    COMMAND
                        ${LLVM_COV_PATH} export $<TARGET_FILE:${TARGET_NAME}> -instr-profile=${target_code_coverage_COVERAGE_TARGET_NAME}.profdata
                        -format="text" ${LINKED_OBJECTS} ${EXCLUDE_REGEX} > ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/${target_code_coverage_COVERAGE_TARGET_NAME}.json
                    DEPENDS ccov-processing-${target_code_coverage_COVERAGE_TARGET_NAME}
                )

                # Generates output of the coverage information for Sonar
                add_custom_target(
                    ccov-${target_code_coverage_COVERAGE_TARGET_NAME}
                    COMMAND
                        ${LLVM_COV_PATH} show $<TARGET_FILE:${TARGET_NAME}> -instr-profile=${target_code_coverage_COVERAGE_TARGET_NAME}.profdata
                        -show-line-counts-or-regions ${LINKED_OBJECTS} ${EXCLUDE_REGEX} >
                        ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/${target_code_coverage_COVERAGE_TARGET_NAME}_coverage.txt
                    DEPENDS ccov-processing-${target_code_coverage_COVERAGE_TARGET_NAME}
                )
            ...
            elseif (CMAKE_C_COMPILER_ID MATCHES "GNU" OR CMAKE_CXX_COMPILER_ID MATCHES "GNU")
                ...
                add_custom_target(
                    ccov-run-${target_code_coverage_COVERAGE_TARGET_NAME}
                    COMMAND ${CMAKE_CROSSCOMPILING_EMULATOR} ${target_code_coverage_PRE_ARGS} $<TARGET_FILE:${TARGET_NAME}> ${target_code_coverage_ARGS}
                    DEPENDS ${TARGET_NAME}
                )
                ...
                if (${CMAKE_VERSION} VERSION_LESS "3.17.0")
                    add_custom_target(
                        ...
                        COMMAND ${CMAKE_CROSSCOMPILING_EMULATOR} ${target_code_coverage_PRE_ARGS} $<TARGET_FILE:${TARGET_NAME}> ${target_code_coverage_ARGS}
                        COMMAND ${LCOV_PATH} --directory ${CMAKE_BINARY_DIR} --base-directory ${CMAKE_SOURCE_DIR} --capture ${EXTERNAL_OPTION} --output-file
                                ${COVERAGE_INFO}
                ...
                add_custom_target(
                    ccov-${target_code_coverage_COVERAGE_TARGET_NAME}
                    COMMAND ${GENHTML_PATH} -o ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/${target_code_coverage_COVERAGE_TARGET_NAME} ${COVERAGE_INFO}
            ...
            add_custom_command(
                ...
                POST_BUILD
                COMMAND ;
                COMMENT "Open ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/${target_code_coverage_COVERAGE_TARGET_NAME}_coverage.txt to view the coverage report."
            )
...
# Adds code coverage instrumentation to all targets in the current directory and any subdirectories. To add coverage instrumentation to only specific targets,
...
# Adds the 'ccov-all' type targets that calls all targets added via `target_code_coverage` with the `ALL` parameter, but merges all the coverage data from them
# into a single large report  instead of the numerous smaller reports. Also adds the ccov-all-capture Generates an all-merged.info file, for use with coverage
# dashboards (e.g. codecov.io, coveralls).
# ~~~
# Optional:
...
function (add_code_coverage_all_targets)
    ...
    if (CODE_COVERAGE)
        if (CMAKE_C_COMPILER_ID MATCHES "(Apple)?[Cc]lang" OR CMAKE_CXX_COMPILER_ID MATCHES "(Apple)?[Cc]lang")
            ...
            if (WIN32)
                add_custom_target(
                    ccov-all-processing COMMAND powershell -Command $$FILELIST = Get-Content ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/profraw.list\;
                                                llvm-profdata.exe merge -o ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/all-merged.profdata -sparse $$FILELIST
                )
            else ()
                add_custom_target(
                    ccov-all-processing COMMAND ${LLVM_PROFDATA_PATH} merge -o ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/all-merged.profdata -sparse `cat
                                                ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/profraw.list`
                )
            endif ()
            ...
            if (WIN32)
                add_custom_target(
                    ccov-all-report
                    COMMAND powershell -Command $$FILELIST = Get-Content ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/binaries.list\; llvm-cov.exe report $$FILELIST
            ...
            if (WIN32)
                add_custom_target(
                    ccov-all-export
                    COMMAND
                        powershell -Command $$FILELIST = Get-Content ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/binaries.list\; llvm-cov.exe export $$FILELIST
            ...
            # Generate output of all added targets for Sonar
            if (WIN32)
                add_custom_target(
                    ccov-all
                    COMMAND
                        powershell -Command $$FILELIST = Get-Content ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/binaries.list\; llvm-cov.exe show $$FILELIST
                ...
                add_custom_target(
                    ...
                    COMMAND
                        ...
                        -instr-profile=${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/all-merged.profdata -show-line-counts-or-regions ${EXCLUDE_REGEX} >
                        ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/all-merged_coverage.txt
                    DEPENDS ccov-all-processing
                )
            endif ()
        ...
        elseif (CMAKE_C_COMPILER_ID MATCHES "GNU" OR CMAKE_CXX_COMPILER_ID MATCHES "GNU")
            ...
            add_custom_target(
                ccov-all
                COMMAND ${GENHTML_PATH} -o ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/all-merged ${COVERAGE_INFO} -p ${CMAKE_SOURCE_DIR}
        ...
        add_custom_command(
            ...
            POST_BUILD
            COMMAND ;
            COMMENT "Open ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/all-merged_coverage.txt to view the coverage report."
        )
    endif ()
endfunction ()
