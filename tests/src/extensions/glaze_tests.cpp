...
TEST_SUITE("can_glaze_de_and_serialize")
{
    TEST_CASE("when_serializing_stronk_integer_then_can_deserialize_to_same_value")
    {
        ...
        REQUIRE(json_str_opt.has_value());
        CHECK_EQ(R"({"a_number":42})", json_str_opt.value());
        auto val_opt = glz::read_json<an_int_can_glaze_de_and_serialize_wrapper>(json_str_opt.value());
        CHECK(val_opt.has_value());
        CHECK_EQ(val, val_opt.value());
    }
    ...
    TEST_CASE("when_serializing_stronk_string_then_can_deserialize_to_same_value")
    {
        ...
        auto json_str_opt = glz::write_json(val);
        REQUIRE(json_str_opt.has_value());
        CHECK_EQ(R"({"a_string":"hello"})", json_str_opt.value());
        CHECK_EQ(val, glz::read_json<a_string_can_glaze_de_and_serialize_wrapper>(json_str_opt.value()).value());
    }
    ...
    TEST_CASE("when_serializing_a_type_with_multiple_members_it_roundtrips")
    {
        ...
        auto json_str_opt = glz::write_json(val);
        REQUIRE(json_str_opt.has_value());
        CHECK_EQ(R"({"val1":24,"val2":"world"})", json_str_opt.value());
        CHECK_EQ(val, glz::read_json<a_class_with_two_can_glaze_members>(json_str_opt.value()).value());
    }
}
