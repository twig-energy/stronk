...
git diff --diff-filter=d --name-only origin/main | grep -E "\.h$" | xargs -r -t -n 1 -P 8 -- clang-tidy-19 --fix --checks="-*,misc-include-cleaner" -p $BUILD_FOLDER
...
# Format after fixing
git diff --diff-filter=d --name-only origin/main | grep -E "\.h$" | xargs -I{} -t -n 1 -P 8 sh -c 'cmake -D FIX=YES -D PATTERNS="{}" -D FORMAT_COMMAND=clang-format-19 -P cmake/lint.cmake'
