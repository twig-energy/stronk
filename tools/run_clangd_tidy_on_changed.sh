...
if [ "$1" != "all" ]; then
    git diff --diff-filter=d --name-only origin/main | grep -E "\.(hpp|cpp)$" | xargs -r -t -- clangd-tidy -p $BUILD_FOLDER --clangd-executable=clangd-19 --github --tqdm
else
    find include source test example -type f | grep -E "\.(hpp|cpp)$" | xargs -r -t -- clangd-tidy -p $BUILD_FOLDER --clangd-executable=clangd-19 --github --tqdm
fi
